...
// TestDeviceHealthEvents_HeartbeatRefreshesLastUpdated verifies that a
// heartbeat advances every device's LastUpdated: the kubelet compares that
// timestamp against the health check timeout, so a resend with the original
// timestamp would still let the device decay to unknown health.
func TestDeviceHealthEvents_HeartbeatRefreshesLastUpdated(t *testing.T) {
	monitor := &mockHealthMonitor{heartbeatCh: make(chan struct{}, 1)}
	stale := healthyGPU0()
	stale.LastUpdated = time.Now().Add(-time.Minute)
	d := newHealthTestDriver(monitor, stale)

	initial, reports, stopWatch := startWatch(t, d)
	defer stopWatch()
	require.Len(t, initial.Devices, 1)
	require.Equal(t, stale.LastUpdated, initial.Devices[0].LastUpdated)
...
// TestInitDeviceHealth_AppliesQueuedEvents verifies that health events the
// monitor queued during registration (devices it could not register are
// unmonitored) are reflected in the seeded map instead of being overwritten
// with healthy, and are retained for taint processing.
func TestInitDeviceHealth_AppliesQueuedEvents(t *testing.T) {
	...
	monitor := &mockHealthMonitor{unhealthyCh: unhealthy}
	d := &driver{
		nodeName:            "node1",
		deviceHealthMonitor: monitor,
		state: &DeviceState{
	...
	assert.Equal(t, kubeletplugin.HealthStatusHealthy, d.deviceHealth["gpu-0"].Health)
	assert.Equal(t, kubeletplugin.HealthStatusUnknown, d.deviceHealth["gpu-1"].Health)
	assert.Contains(t, d.deviceHealth["gpu-1"].Message, "not monitored")
	assert.Empty(t, unhealthy, "queued events must be drained")
	require.Len(t, d.pendingHealthEvents, 1)
	assert.Equal(t, HealthEventUnmonitored, d.pendingHealthEvents[0].EventType)

	// The initial snapshot for an early subscriber reflects the queued event.
	initial, _, stopWatch := startWatch(t, d)
	defer stopWatch()
	byName := map[string]kubeletplugin.HealthStatus{}
	for _, dev := range initial.Devices {
		byName[dev.DeviceName] = dev.Health
	}
	assert.Equal(t, kubeletplugin.HealthStatusHealthy, byName["gpu-0"])
	assert.Equal(t, kubeletplugin.HealthStatusUnknown, byName["gpu-1"])
}

// TestDeviceHealthEvents_ProcessesPendingEvents verifies that events drained
// during initDeviceHealth are handed to the taint path when the consumer
// starts and are not processed twice. The device already carries the taint,
// so no ResourceSlice republish (which needs a live plugin helper) occurs.
func TestDeviceHealthEvents_ProcessesPendingEvents(t *testing.T) {
	gpu0 := &AllocatableDevice{Gpu: &GpuInfo{minor: 0}}
	monitor := &mockHealthMonitor{heartbeatCh: make(chan struct{}), unhealthyCh: make(chan *DeviceHealthEvent)}
	d := newHealthTestDriver(monitor, healthyGPU0())
	d.state.perGPUAllocatable.allocatablesMap["0000:01:00.0"]["gpu-0"] = gpu0
	event := &DeviceHealthEvent{Devices: []*AllocatableDevice{gpu0}, EventType: HealthEventUnmonitored}
	require.True(t, gpu0.AddOrUpdateTaint(healthEventToTaint(monitor, event)))
	d.pendingHealthEvents = []*DeviceHealthEvent{event}
	...
	assert.Nil(t, d.pendingHealthEvents, "pending events must be consumed exactly once")
	require.Len(t, gpu0.taints, 1)
	assert.Equal(t, TaintKeyUnmonitored, gpu0.taints[0].Key)
}
