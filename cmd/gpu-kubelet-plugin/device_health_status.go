...
//
// The NVML monitor may already have queued health events during event
// registration (devices it failed to register are reported as unmonitored).
// Those are applied to the map synchronously here, before the health service
// is advertised, so an early kubelet subscription does not see a device as
// healthy when the monitor already knows it cannot observe it. The events are
// kept in pendingHealthEvents for deviceHealthEvents, which still has to
// translate them into ResourceSlice taints once the initial slices are
// published.
func (d *driver) initDeviceHealth() {
	...
	for {
		...
		case event, ok := <-d.deviceHealthMonitor.Unhealthy():
			...
			d.pendingHealthEvents = append(d.pendingHealthEvents, event)
		default:
			return
		}
	}
...
// refreshDeviceHealth marks every device's current health as freshly
// determined and notifies all pending WatchHealthStatus subscribers. It is
// called on each heartbeat from the NVML monitor's event loop: the kubelet
// compares each device's LastUpdated against its health check timeout, so a
// resend must also advance the timestamps or the devices would still decay
// to an unknown health status while the monitor is alive.
func (d *driver) refreshDeviceHealth() {
	now := time.Now()
	d.healthMu.Lock()
	for name, health := range d.deviceHealth {
		health.LastUpdated = now
		d.deviceHealth[name] = health
	}
	d.healthMu.Unlock()
